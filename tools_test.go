...
// loopfunc is used to satisfy the interface requirements for http.Client
type HttpLoopfunc func(req *http.Request) *http.Response

// RoundTrip is used to satisfy the interface requirements for http.Client
func (f HttpLoopfunc) RoundTrip(req *http.Request) (*http.Response, error) {
	return f(req), nil
}

// NewTestClient returns *http.Client with Transport replaced to avoid making real calls
func NewTestClient(fn HttpLoopfunc) *http.Client {
	return &http.Client{
		Transport: fn,
	}
}
...
func TestTools_DownloadFile(t *testing.T) {
	...
	testApp.DownloadFile(rr, req, "./testdata", "img.jpg", "exhaust.jpg")

	res := rr.Result()
	defer res.Body.Close()

	if res.Header["Content-Length"][0] != "1037088" {
		t.Error("wrong content length of", res.Header["Content-Length"][0])
	}

	if res.Header["Content-Disposition"][0] != "attachment; filename=\"exhaust.jpg\"" {
...
func TestTools_UploadFile(t *testing.T) {
	...
	go func() {
		...
		part, err := writer.CreateFormFile("file", "./testdata/ds.png")
		if err != nil {
			t.Error(err)
		}

		f, err := os.Open("./testdata/ds.png")
		if err != nil {
			t.Error(err)
		}
		defer f.Close()
...
func TestTools_CreateDir(t *testing.T) {
	...
	err := testTool.CreateDir("./testdata/testDir")
	if err != nil {
		t.Error(err)
	}
